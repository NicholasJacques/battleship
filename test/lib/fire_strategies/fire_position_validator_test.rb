...
describe FirePositionValidator do
  describe '.validate_fire_position' do
    it "accepts a valid position" do
      assert_nil FirePositionValidator.validate_fire_position(position: "A1", board: Board.new)
    end

    it "rejects a position that is not on the board" do
      error = assert_raises(FirePositionError) { FirePositionValidator.validate_fire_position(position: "A11", board: Board.new) }
    ...
    it "rejects a position that has already been fired upon" do
      ...
      error = assert_raises(FirePositionError) { FirePositionValidator.validate_fire_position(position: "A1", board: board) }
