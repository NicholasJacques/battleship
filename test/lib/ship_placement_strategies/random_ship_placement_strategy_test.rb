...
describe RandomShipPlacementStrategy do
  ...
  describe '.coordinates_for_ship_in_direction' do
    it "selects the appropriate cells to the right" do
      # skip
      board = Board.new
      result = RandomShipPlacementStrategy.coordinates_for_ship_in_direction([0,0], :right, 4, board)
    ...
    it "selects the appropriate cells to the left" do
      # skip
      board = Board.new
      result = RandomShipPlacementStrategy.coordinates_for_ship_in_direction([3,0], :left, 4, board)
      assert_equal %w(A1 A2 A3 A4), result
    end
    ...
    it "selects the appropriate cells to the south" do
      # skip
      board = Board.new
      result = RandomShipPlacementStrategy.coordinates_for_ship_in_direction([0,0], :down, 4, board)
    ...
    it "selects the appropriate cells to the north" do
      # skip
      board = Board.new
      result = RandomShipPlacementStrategy.coordinates_for_ship_in_direction([0,3], :up, 4, board)
      assert_equal %w(A1 B1 C1 D1), result
    end
    ...
    it "raises an error if cells are not on the board to the left" do
      # skip
      board = Board.new
      assert_raises(ShipPlacementError) { RandomShipPlacementStrategy.coordinates_for_ship_in_direction([0,0], :left, 4, board) }
    end

    it "raises an error if cells are not on the board to the north" do
      # skip
      board = Board.new
      assert_raises(ShipPlacementError) { RandomShipPlacementStrategy.coordinates_for_ship_in_direction([0,0], :up, 4, board) }
    end

    it "raises an error if cells are not on the board to the right" do
      # skip
      board = Board.new
      assert_raises(ShipPlacementError) { RandomShipPlacementStrategy.coordinates_for_ship_in_direction([9,9], :right, 4, board) }
    end

    it "raises an error if cells are not on the board to the south" do
      # skip
      board = Board.new
      assert_raises(ShipPlacementError) { RandomShipPlacementStrategy.coordinates_for_ship_in_direction([9,9], :down, 4, board) }
    end

    it "can place ship up to the edge of the board" do
      # skip
      board = Board.new
      assert_equal %w[J6 J7 J8 J9 J10], RandomShipPlacementStrategy.coordinates_for_ship_in_direction([5,9], :right, 5, board)
    end

    it "raises an error if only one cell is off the board" do
      # skip
      board = Board.new
      assert_raises(ShipPlacementError) { RandomShipPlacementStrategy.coordinates_for_ship_in_direction([6,9], :right, 5, board) }
    end
  end
end